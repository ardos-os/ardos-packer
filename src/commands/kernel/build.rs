...
pub fn build(manifest: &Manifest) -> Result<KernelBuildResult, KernelBuildError> {
	...
	// --- Calculate rebuild hash ---
	// The kernel source tarball alone is not enough: changing manifest
	// options must invalidate the cached kernel artifact too.
	let current_hash = format!(
		"source={}\noptions={}",
		hash_file(&tarball_path)?,
		hash_file(&options_path)?
	);
	...
	if hash_path.exists() {
		...
		if old_hash.trim() == current_hash {
			...
			println!(
				"{} {}",
				"󰞇 Kernel source unchanged, skipping rebuild →"
					.yellow()
					.bold(),
